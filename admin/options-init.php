...
$hipoul_defaults = apply_filters( 'graphene_defaults', $hipoul_defaults );
function graphene_get_settings(){
	global $hipoul_defaults;
	$hipoul_settings = array_merge( $hipoul_defaults, (array) get_option( 'graphene_settings', array() ) );
	return apply_filters( 'graphene_settings', $hipoul_settings );
}
global $hipoul_settings;
$hipoul_settings = graphene_get_settings();
...
//<![CDATA[
    ...
    var graphene_settings = <?php echo json_encode($hipoul_settings); ?>;
//]]>
</script>
        <?php
    }
...
function graphene_page_template_visualizer() {  
    global $hipoul_settings, $post_id;
	...
	if ( ! get_post_meta( $post_id, '_wp_page_template', true ) ){
		...
		switch( $hipoul_settings['column_mode']){
			case 'one_column':
