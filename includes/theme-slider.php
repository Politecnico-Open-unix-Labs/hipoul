...
if ( ! function_exists( 'graphene_scrollable' ) ) :
	function graphene_scrollable() { 
		...
		$interval = ( $hipoul_settings['slider_speed'] ) ? $hipoul_settings['slider_speed'] : 7000;
                $speed = $hipoul_settings['slider_trans_speed'];
		?>
            <!-- Scrollable -->
            <script type="text/javascript">
				...
				<?php if ( $hipoul_settings['slider_animation'] == 'horizontal-slide' ) : ?>
					$("#slider_root")
						.scrollable({
							circular: true,
							clickable: false,
				...
				<?php else : 
						if ( $hipoul_settings['slider_animation'] == 'vertical-slide' )
							$effect = 'slide';
						if ( $hipoul_settings['slider_animation'] == 'fade' )
							$effect = 'fade';
						if ( $hipoul_settings['slider_animation'] == 'none' )
							$effect = 'default';
				?>
...
function graphene_slider(){
    ...
    <div class="<?php echo $class; ?>">
        ...
        while ( $sliderposts->have_posts() ) : $sliderposts->the_post();
            ...
            <div id="slider-post-<?php the_ID(); ?>" <?php echo $style; ?>>
                <?php do_action( 'graphene_before_sliderpost' ); ?>
                
                <?php if ( $hipoul_settings['slider_display_style'] == 'bgimage-excerpt' ) : ?>
                	<a href="<?php echo $slider_link_url; ?>" class="permalink-overlay"><span><?php _e( 'View full post', 'graphene' ); ?></span></a>
                <?php endif; ?>
                
                <?php if ( $hipoul_settings['slider_display_style'] == 'thumbnail-excerpt' ) : ?>
					<?php /* The slider post's featured image */ ?>
                    <?php 
                    if ( get_post_meta( get_the_ID(), '_graphene_slider_img', true) != 'disabled' && ! ( ( get_post_meta( get_the_ID(), '_graphene_slider_img', true ) == 'global' || get_post_meta( get_the_ID(), '_graphene_slider_img', true ) == '' ) && $hipoul_settings['slider_img'] == 'disabled' ) ) : 
                ...
                <div class="slider-entry-wrap clearfix">
                	<div class="slider-content-wrap">
                        ...
                        <div class="slider_post_entry clearfix">
                        	<?php 
							if ( $hipoul_settings['slider_display_style'] != 'full-post' ){
								the_excerpt(); 
							?>
                            <?php if ( $hipoul_settings['slider_display_style'] == 'thumbnail-excerpt' ) : ?>
...
if ( ! $hipoul_settings['slider_disable'] ){
	if ( ! $hipoul_settings['slider_position'] )
...
if ( ! function_exists( 'graphene_get_slider_image' ) ) :
	function graphene_get_slider_image( $post_id = NULL, $size = 'thumbnail', $urlonly = false, $default = '' ){
		...
		$global_setting = ( $hipoul_settings['slider_img'] ) ? $hipoul_settings['slider_img'] : 'featured_image';
		...
		if ( $final_setting == 'disabled' ){					// image disabled
			...
			if ( has_post_thumbnail( $post_id ) ) :
				...
				$html = hipoul_get_post_image( $post_id, $size, '', $urlonly);
			...
			if (!$urlonly){
				...
				if ( $local_setting != 'global' ) :
					...
					$html .= '<img src="'.$hipoul_settings['slider_imgurl'].'" alt="" />';
				endif;
				$html .= '</a>';
			} else {
				if ( $local_setting != 'global' ) :
					...
					$html .= $hipoul_settings['slider_imgurl'];
				endif;
			}
...
function graphene_get_slider_posts(){
	...
	$slidertype = ( $hipoul_settings['slider_type'] != '' ) ? $hipoul_settings['slider_type'] : false;
	...
	$postcount = ( $hipoul_settings['slider_postcount'] ) ? $hipoul_settings['slider_postcount'] : 5 ;
	...
	if ( $slidertype && $slidertype == 'posts_pages' ) {                    
		$post_ids = $hipoul_settings['slider_specific_posts'];
	...
	if ( $slidertype && $slidertype == 'categories' && is_array( $hipoul_settings['slider_specific_categories'] ) ) {                        
		$args = array_merge( $args, array( 'category__in' => $hipoul_settings['slider_specific_categories'] ) );
		
		if ( $hipoul_settings['slider_random_category_posts'] )
			$args = array_merge( $args, array( 'orderby' => 'rand' ) );
	}
...
function graphene_exclude_slider_categories( $request ){
	...
	if ( $hipoul_settings['slider_exclude_categories'] != $hipoul_defaults['slider_exclude_categories'] ){
		$dummy_query = new WP_Query();
    	$dummy_query->parse_query( $request );
		
		if ( ( $hipoul_settings['slider_exclude_categories'] == 'everywhere' ) || 
				$hipoul_settings['slider_exclude_categories'] == 'homepage' && $dummy_query->is_home() )
			$request['category__not_in'] = $hipoul_settings['slider_specific_categories'];
	}
