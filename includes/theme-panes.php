...
function graphene_homepage_panes(){
	...
	if ( $hipoul_settings['show_post_type'] == 'latest-posts' || $hipoul_settings['show_post_type'] == 'cat-latest-posts' ){
		$pane_count = $hipoul_settings['homepage_panes_count'];
	} elseif ( $hipoul_settings['show_post_type'] == 'posts' ) {
		$pane_count = count(explode( ',', $hipoul_settings['homepage_panes_posts']) );
	}
	...
	if ( $hipoul_settings['show_post_type'] == 'latest-posts' ){
		$args_merge = array(
	...
	if ($hipoul_settings['show_post_type'] == 'cat-latest-posts' ){
		$args_merge = array(
							'category__in' => $hipoul_settings['homepage_panes_cat'],
							);
		$args = array_merge( $args, $args_merge );
	}
	
	// args specific to posts/pages
	if ( $hipoul_settings['show_post_type'] == 'posts' ){
		
         $post_ids = $hipoul_settings['homepage_panes_posts'];
	...
	?>
		<div id="homepage-pane-<?php the_ID(); ?>">
			...
			if ( has_post_thumbnail( get_the_ID() ) ) {
				...
				echo hipoul_get_post_image( get_the_ID(), 'graphene-homepage-pane', 'excerpt' );
			}
			?>
            </a>
...
function graphene_display_homepage_panes(){
	...
	if ( get_option( 'show_on_front' ) == 'page' && ! $hipoul_settings['disable_homepage_panes'] && is_front_page() ) {
