...
function graphene_enqueue_styles(){
	...
	if ( ! is_admin() ){
		...
		if ( $hipoul_settings['light_header'] ) wp_enqueue_style( 'graphene-light-header' );
		if ( is_singular() && $hipoul_settings['print_css'] ) wp_enqueue_style( 'graphene-print' );
...
function graphene_enqueue_scripts(){
	...
	if ( ! is_admin() ) { // Front-end only
		...
		if ( is_front_page() && ! $hipoul_settings['slider_disable'] )
