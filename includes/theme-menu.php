...
*/
class Hipoul_Walker_Nav_Menu extends Walker_Nav_Menu {
...
function add_search_to_wp_menu ( $items, $args ) {
	if( 'header-menu' === $args -> theme_location ) {
		...
		$items .= '<form method="get" class="navbar-form menu-search-form" role="search" action="' . get_bloginfo('home') . '/">';
		$items .= '<div class="input-group"><input type="text" placeholder="Search" name="s" id="s" class="form-control pull-right" placeholder="Search...">';
		$items .= '<span class="input-group-btn"><button type="submit" class="btn btn-default search-submit" id="searchsubmit"><span class="glyphicon glyphicon-search"><span class="sr-only">Search</span></span></button></span></div>';
		$items .= '</form></li>';
	}
return $items;
}
...
if ( ! function_exists( 'graphene_default_menu' ) ) :
	...
	function graphene_default_menu(){ global $hipoul_settings; ?>
		...
		<ul id="header-menu" class="<?php echo graphene_get_menu_class('menu clearfix default-menu'); ?>">
            ...
            <li <?php if ( is_single() || is_front_page() ) { echo 'class="current_page_item current-menu-item"'; } ?>>
            	<a href="<?php echo get_home_url(); ?>">
                    ...
                    <?php if ( $hipoul_settings['navmenu_home_desc']) {echo '<span>'.$hipoul_settings['navmenu_home_desc'].'</span>';} ?>
                </a>
            </li>
            <?php endif; ?>
            <?php 
...
class Walker_PageDescription extends Walker_Page {
    ...
    function start_el( &$output, $page, $depth, $args, $current_page ) {
		...
		if ( ! $depth && ! $hipoul_settings['disable_menu_desc'] ){
			$menu_title = '<strong>' . $menu_title . '</strong>';
		}
		if ( ! $hipoul_settings['disable_menu_desc'] ){
...
function graphene_get_menu_class( $menu_class ){
    ...
    if (($search_box_location = $hipoul_settings['search_box_location']) && $search_box_location == 'nav_bar'){
        $menu_class .= ' grid_12';
    }
